classdef matrixnormalWishart < handle  
    properties
        ...
        % observations, i.e. CP = matrixnormal(M,U,V)*Wishart(invU)
        % where V is the covariance of the regressions and U is the 
        % covariance of the observation noise 
        
        % This version of the routine assumes posterior of the form 
        % q(X|mu,V,invU)q(invU)
        ...
        invU  % is Wishart distribution on n x n symmetric positive definite
    ...
    methods               
        function self = matrixnormalWishart(M_0,U_0,V_0)  %neff>1
        ...
        function res = variance(self)
            res = diag(self.invU.ESigma)*diag(self.V)';
        end
