classdef PoiNLregression < handle
    
    % Poisson non-linear regression.  Same structure as NLregression.m, but
...
% ELBO
    ...
    methods
        function self = PoiNLregression(NR,NC,D,alpha,iswhite,muY,chunkfactor)
            NO=1;
            self.iters=0;
...
%                fprintf([num2str(i/niters*100),' Percent Complete\n'])
        ...
        function [DL,p,logptilde,etaugz] = update_suffstats(self,Y,X,updateSEXX,p)
            ...
            if(~exist('self.SlnYfactorial','var'))
                self.SlnYfactorial = sum(gammaln(Y+1));
            end
            if(updateSEXX==1)
                self.SlnYfactorial = sum(gammaln(Y+1));                
                self.chunkSEXX(X);
...
%                 if(mod(i,5)==1 &  i < 50  &  filliters>0)
                if(i < 0.5*iters & filliters>0 & mod(i,10)==0)
...
%                    c=c(:,1:end-1);
                    [z,c]=kmeans(Upred.mu(:,idxe)',NU);                    
                else
%                    c=mean(X(idxe(1:Ns),:)*self.getfilter()');
                    c=mean(Upred.mu(:,idxe)');
                    z=ones(Ns,1);
                end
                for k=1:NU
        ...
        function [Evar,EDev,KLqprior] = plot(self,Y,X,fignum)
            ...
            if(self.D>1)
                ...
                for j=i+1:self.D
                    figure(fignum)
                    clf
                    scatter3(Upred.mu(i,:),Upred.mu(j,:),Ypred.mu(1,:),9*ones(size(Ypred.mu(1,:))),cc(idx,:),'.')                
                    xlabel(num2str(i))
                    ylabel(num2str(j))
                    fignum=fignum+1;
                end
                ...
                figure(fignum)
                fignum=fignum+1;
                scatter(Upred.mu(1,:),Ypred.mu(1,:),9*ones(size(Ypred.mu(1,:))),cc(idx,:),'.')                
            end
...
%            Abar = self.AB{idx}.mu(1:self.D);            
        ...
        function canoncorrUY(self,Y,X,logswitch)
            % Linearally adjusts u's according to Ru or 
            % Finds the rotation on u that maximizes the correlations 
            % between u and Y (or logY)
 
            self.orthogonalizeW(1);
            [Ypred,Upred]=self.getPredictions(Y,X);

            if(~exist('logswitch','var'))
                logswitch=0;
            end
            if(logswitch)
                [A,B]=canoncorr(log(Ypred.mu'),Upred.mu');
            else
                [A,B]=canoncorr((Ypred.mu'),Upred.mu');
            end
            B=B/A;
            B=B'/norm(B);
            for i=2:self.D
        ...
        function rotateU(self,R) 
            invP=R;
            P=inv(R);
            
            self.Sigmau = invP*self.Sigmau*invP';
            self.invSigmau = P'*self.invSigmau*P;
            self.logdetinvSigmau = self.logdet(self.invSigmau);
            ...
            for k=1:self.NC
                ...
                self.u{k}.mu_0 = invP*self.u{k}.mu_0;
                self.u{k}.invSigma_0 = P'*self.u{k}.invSigma_0*P;
                self.u{k}.Sigma_0 = inv(self.u{k}.invSigma_0);
                self.u{k}.invSigmamu_0 = self.u{k}.invSigma_0*self.u{k}.mu_0;
            ...
            if(self.iswhite)
                ...
                self.W.mu_0 = self.W.mu_0*P;
                self.W.V_0 = P'*self.W.V_0*P;
                self.W.invV_0 = invP*self.W.invV_0*invP';
            end
            
            P(self.D+1,self.D+1)=1;
            invP(self.D+1,self.D+1)=1;
            for k=1:self.NC
                self.AB{k}.mu = self.AB{k}.mu*P;
                self.AB{k}.invV = invP*(self.AB{k}.invV)*invP';
                self.AB{k}.V=inv(self.AB{k}.invV);

                self.AB{k}.mu_0 = self.AB{k}.mu_0*P;
                self.AB{k}.invV_0 = invP*(self.AB{k}.invV_0)*invP';
                self.AB{k}.V_0=inv(self.AB{k}.invV_0);

            end
            
        end
        
        function projectontoW(self,W,mask,V,D) % here W is the linear receptive field (assumed a row)
                                           % and mask is the set of W's we wish to consider
                                           % for the projection as indicated by a boolean
            
            temp=self.getfilter();                                           
            if(exist('D','var'))
               temp=temp*V'*diag(sqrt(D));
            end
            if(~exist('mask','var'))
                mask=W<Inf;
            end
...
%            W=W*V'*diag(sqrt(D));
        ...
        function orthogonalizeW(self,Widx,V,D) % here idx is the linear receptive field.
...
%             end
        ...
        function orthogonalizeU(self,Y,X)
            [Ypred,Upred]=self.getPredictions(Y,X);
            CC=cov(Upred.mu');
            [V,D]=eig(CC,'vector');
            self.rotateU(diag(1./sqrt(D))*V');
        end
